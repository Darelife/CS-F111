 ...
 *
 * Requires: The student names have no spaces. Can contain uppercase, lowercase
 * characters and digits. Names will start with either uppercase or lowercase
 * characters.
 *
 * Example 1:
...
int num_names(struct student s[], int len, char key) {
  ...
  for (int i = 0; i < len; i++) {
    char newKey = key, v = s[i].name[0];

    if (newKey >= 65 && newKey <= 90)
      newKey += 32;
    if (v >= 65 && v <= 90)
      v += 32;
...
void sort_descending(struct student s[], int len) {
  for (int i = 1; i < len; i++) {
    ...
    int point2 = i;
    int point1 = i - 1;
    while (point1 >= 0 && s[point1].total <= s[point2].total) {
      // Swap
      struct student temp = s[point2];
      s[point2] = s[point1];
      s[point1] = temp;
...
/**
 * @brief Populates the given array of students with the user's input and
 * returns the number of students.
 *
 * Calculate the total marks for the student correctly.
 */
...
int get_input(struct student s[]) {
  ...
  for (int i = 0; i < num; i++) {
    printf("Student %d\n", i + 1);
    printf("Enter Student Name, Roll no, Marks of Subject 1, and Marks of "
           "Subject 2:\n");
    scanf("%s", s[i].name);
    scanf("%d", &s[i].rollno);
    scanf("%d", &s[i].marks[0]);
    ...
    // Calculate the total marks correctly
