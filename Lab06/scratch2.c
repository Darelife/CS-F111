...
char calculator(int balance) {
  printf("Balance: %d\n",
         balance); // This statement needs to be there in the function. You can
                   // however change the position of this statement.

  char option;
  scanf(" %c", &option);
  int callBack = balance;

  if (option == 'e' || option == 'c' || (option != '+' && option != '-'))
    return option;
  if (option == '+') {
    int value;
    scanf("%d", &value);
  ...
  char returnedValue = calculator(balance);
  if (returnedValue == 'c')
    calculator(callBack);
  else
    return 'e';
