 ...
 *
 * Takes in the number of employees as the parameter. 'num' is a positive
 * integer. Returns the pointer to the organization.
 *
 * Set the 'num' field of the organization as the value of the function
 * parameter. Allocate 'e' field as the starting address of an array that holds
 * the pointers to each individual employee.
 *
 ...
 * - This function would allocate memory for the organization in the heap
 * starting at 0x50
 ...
 *
 * Requires: Employee names contains only upper case and lower case characters.
 * No spaces or digits or special characters. Assume that the length of all
 * entered names are <= 49 (that is, no invalid input will be provided)
 *
 ...
 * - Save that address as the value of the first index of the array of pointers
 * to the employees created in the allocate_memory()
 * - Similarly, the second index will store 0x3000 for the second employee and
 * the third index stores 0x4000 which points to the third employee.
 * - The first employee's name is "Arnab" and the productivity is 50. Similarly
 * the others.
 */
void get_input(org *o) {
  for (int i = 0; i < o->num; i++) {
    // TODO: allocate memory for an employee and store the starting address in
    // the respective index of the array created in the allocate_memory()

    printf("Enter name and productivity:\n");

    // TODO: Complete the 2 scanf statements to take the name and productivity
    // as input.
        scanf("%s", //TODO
        scanf("%d", //TODO
  }
 ...
 * If more than one employee has the same highest productivity, then return the
 * larger index.
 *
 * Assume that the employee index start from 0 in the array of pointers each
 * poinitng to an employee.
 *
 * Examples:
 *      e[0]->prod = 50
 *      e[1]->prod = 100
 ...
 * the lower case.
 *
 * The index of the array containing the pointers to the employees start from
 * zero
 *
 * Examples:
 *      e[0]->fname = "abBA"
...
int main() {
  ...
  if (operation == 1) {
    ...
    printf("The characters in the longest employee's name in alphabetical "
           "order are %s\n",
           o->e[index]->fname);
  }
