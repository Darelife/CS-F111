...
/**
 * Returns the index at which the given `key` is found in the non-decreasing,
 * non-empty array `nums` of size `length`. -1 if `key` is not found. It
 * performs the search using a binary search algorithm and prints the index it
 * is checking in each step.
 *
